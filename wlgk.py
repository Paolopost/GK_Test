...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        sys.stderr.write("Usage: %s <filename-graph-1> <filename-graph-2>\n" %
                         (os.path.basename(sys.argv[0]),))
        sys.exit(1)
    if not os.path.exists(sys.argv[1]):
        sys.stderr.write("ERROR: Filename %s was not found!\n" % (sys.argv[1],))
        sys.exit(1)
    ...
    for src,g in [arg1, g1], [arg2, g2]:
        num = -1
        lab = {}
        for pair in [line.strip() for line in open(src)]:
            p1, p2 = pair.split()
            if not lab.has_key(p1):
                num += 1
                lab[p1] = num
            if not lab.has_key(p2):
                num += 1
                lab[p2] = num
            g.add_edge(lab[p1], lab[p2])

    gk = GK_WL()
    print gk.compare_normalized(g1, g2, 1, False)
